...
An interrupted remote move may leave both its old path and an already-copied target. Automatic completion requires an unchanged accepted source, the same live S3 Entry renamed with unchanged content, and one unowned target with matching verified bytes. Other identities, changed copies, case aliases, and deferred/unmaterialized Entries still require review. The remote recovery is authenticated and both local paths are checked again; the existing target is not overwritten, and only the old source is retired through local Trash. This lets unrelated updates resume without publishing a duplicate Entry.

Plugin-private staging moves are not user renames. Rename events involving this plugin's staging directory are ignored, and legacy pending staging records are removed on load without changing the Sync Cache or file contents. Real user renames remain tracked, including during synchronization and across excluded paths.

After a successful synchronization saves its accepted state, desktop cleanup may remove empty ancestors of files retired by that run, deepest first. It preserves the Vault root, configuration and excluded subtrees, links, and any real directory contents, including hidden files. The operation uses directory-only non-recursive `rmdir`, identity checks, and the Vault mutation lock; unsupported capabilities or filesystem errors leave directories in place. Untouched empty folders are not swept. **Automatic folder cleanup remains disabled on Android/iOS**: the inspected mobile adapter forces recursive deletion, so its `rmdir(path, false)` is not a safe primitive. Existing empty folders may still need manual removal.
