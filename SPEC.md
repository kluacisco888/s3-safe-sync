...
16. A partial cache created by a manual action retains `bootstrapPending` until ordinary reconciliation completes without unknown local paths. Manual import, download, restore, and review accept only selected Entries, so other local files continue to require Import confirmation across restart or rename. Existing complete caches need no reset. Review ignores historical deletion records when exactly one current Entry owns the requested path; its ID remains bound into the review token. Copy names are bounded to 255 UTF-8 bytes, preserve complete Unicode characters and the ordinary extension, and omit an extension only when it cannot fit alongside the unique suffix.
