...
- Show the current phase, file count, byte count, current path, Head retry reason, and retry delay during metadata checking, content hashing, uploading, downloading, and contention backoff in plugin settings and the desktop status bar without routine success popups. Hash progress UI updates are rate-limited while preserving phase changes and completion.
