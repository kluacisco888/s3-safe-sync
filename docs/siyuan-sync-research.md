...
6. 对当前插件最值得立即移植的是“重要请求不得丢失、冗余轮询不得自我续跑”的请求合并状态机。修复前，`syncNow()` 在已有同步运行时只返回同一个 Promise，没有记录“完成后必须再跑一次”；如果本地修改事件在长同步期间触发，5 秒定时器到期后会被吞并，只能等待下一次 2 分钟轮询。后续又发现，若所有轮询都无条件置位，单轮超过 2 分钟时会永远排着下一轮。现在本地修改、手动操作和退出请求会保留后续轮次，纯定时或回到前台的 Head 轮询则在同步或 Head 退避已进行时跳过。
...
这比修复前插件的单个 `runningSync` Promise 更可靠。本次已加入串行请求队列：运行中收到本地事件、手动同步或退出请求时只置位；当前轮结束后继续执行，直到没有新的重要请求。定时检查和回到前台只用于发现远端变化，已有同步或 Head 退避已覆盖这项工作时直接跳过，避免长任务不断给自己续排下一轮。批量删除确认是安全例外：若同步正在运行，必须等新计划生成后重新确认，避免把旧授权用于已经变化的删除集合。
...
1. **可靠合并同步请求。** 已增加串行请求队列。同步运行中收到本地变更、手动操作或退出请求时，会标记后续轮次；当前轮结束后继续执行，直到重要请求已全部覆盖。纯定时检查和回到前台的 Head 轮询在同步或 Head 退避已进行时跳过，避免单轮超过轮询周期后永远无法回到空闲状态。
2. **为触发合并补集成测试。** 已覆盖第一轮同步完成发布但仍被人为阻塞、电脑再次修改并请求同步、第二轮发布最新内容、另一个 Replica 最终下载最新版的完整链路。
3. **完整元数据扫描 + 增量内容哈希。** 每轮（包括手动“立即同步”）仍列出整个同步范围以发现新增、删除和重命名；只有新文件、`mtime + size` 变化和最近收到文件事件的路径读取内容并计算 SHA-256。缓存丢失、手动“完整性检查”和可配置的 1/7/14/30 天周期（默认 7 天）会重新哈希全部合格文件。桌面扫描以 8 MiB 分块流式计算并主动让出 UI；任何远端覆盖或删除前仍独立读取并校验目标内容。
4. **防止同步期间的新编辑被清除。** 脏路径使用单调版本号；一次同步只确认它开始时捕获的版本，运行中到达的新事件必然保留到下一轮。
5. **保留校验与重命名证据。** 同一次 Head CAS 重试链复用首轮已经验证且元数据未变的内容哈希，避免反复读取整个 Vault；未完成的完整哈希校验和带 Entry ID 的 old→new 重命名关系写入本地插件状态，重启后仍不会把旧路径上的新文件误认成原 Entry。
...
3. 网络瞬态错误采用有上限的指数退避和抖动；由下一轮完整元数据扫描与增量内容哈希恢复，不维护庞大的逐文件待传输清单。
