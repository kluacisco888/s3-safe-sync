...
const localIssueHelp = (issue: LocalSyncIssue): string => {
  switch (issue.kind) {
    case "bootstrap-mismatch": return "This device has no accepted common version for this path, which can happen on first connection, after reinstalling, or after losing local sync records. Compare and resolve shows the differences before you choose to preserve your local version as a new file and receive the remote state. Clearing the cache cannot establish which version you intended.";
